...
class HideModel(DeleteModel):
    """
    Remove a model from the model state without changing the database.
    """

    def describe(self):
        return "Hide model %s" % self.name
...
class RemoveHiddenModel(Operation):
    """
    Remove a model from the database without modifying model state, such as a model that
    has been previously hidden.

    The model will not be recovered after it is removed.
    """
    ...
    def describe(self):
        return "Remove table %s" % self.table_name

    def deconstruct(self):
        return (self.__class__.__qualname__, [], {'table_name': self.table_name})
    ...
    def database_forwards(
        ...
        sql = schema_editor.sql_delete_table % {
